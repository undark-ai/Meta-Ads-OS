...
**Agent recommendation**: for an audit, reach for **Composio** first — it is rung 2 of the connector ladder and is where GA4, Google Ads and other non-native sources are usually already authorised (see `.claude/skills/mcp-discovery`). Supermetrics and Coupler.io are reporting pipelines: useful for scheduled cross-platform extracts and BI feeds, not the first stop when an audit needs one figure from one source.
