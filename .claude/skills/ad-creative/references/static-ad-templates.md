...
## Design Density: When the Headline Is the Creative

Several templates below (Headline Statement, Stat Callout, Us vs. Them, Competitor Callout) are text-led — the type *is* the image. Practitioner-reported distributions from high-performing text-led statics:

- **Dark or high-contrast backgrounds ~40%, bright saturated colour ~35%, light or white ~15–20%.**
- **Headline text occupying 50–70% of the image area.** Not a caption over a photo. If the type is small enough to sit politely in a corner, this is the wrong template.
- **One sentence.** Two claims means two ads.
- **Legible at thumbnail size** — shrink it to the size it actually renders at in a feed before approving it.

Include one sentence, a small corner logo, optionally an embedded CTA, and one accent colour. Exclude bullet lists, stock photography, multi-element layouts, feature lists, and small text.

**The e-commerce caveat.** These ratios come from professional-network feeds, where a wall of type stands out against headshots and articles. In a shopping feed — Instagram, Reels, TikTok — **product-hero and UGC-native imagery frequently beat text-led statics**, because the viewer is browsing and wants to see the product. Text-led templates earn their place for problem-first and category-challenging angles, and lose for anything where the product's appearance is the sell. Test both rather than assuming either.

## Pre-Launch Copy Audit

Five layers, in this order. A failure at any layer sends the concept back rather than forward.

1. **Factual accuracy** — every claim, stat, review count and named person is real, current, and something the brand has authorized. No invented proof. Regulated categories (supplements, skincare efficacy, health, financial) get checked against what the brand can actually substantiate.
2. **Tone** — matches the brand voice document, not generic direct-response voice. Read it aloud; if you wouldn't say it to a customer, it fails.
3. **Structure** — one idea, one reader, one ask. The headline and the headline *field* say different things. Must-see information isn't hiding in the description field, which doesn't render everywhere.
4. **Design density** — the rules above. Legible at thumbnail. Nothing in a placement's safe zone.
5. **Audience fit** — the concept names a real situation from the persona library, and a non-buyer seeing it would scroll past. If a generic viewer might stop, the creative isn't doing any targeting.

Also confirm: AI-generated imagery is labeled, per Meta's disclosure requirement.
